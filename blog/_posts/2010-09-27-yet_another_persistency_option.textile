---
layout: blogpost
title: Yet Another Persistency Option
author: Shay Banon
nick: kimchy
---
...
p. The shared storage option has several benefits. One obvious one is the ability to store (parts) of the index in memory, and still maintain long term persistency in case of full cluster shutdown. It also provides a native solution if backup is required of the indices (to s3 for example).

p. It does comes with an overhead though, the first is the actual need for a shared storage solution for long term persistency, which does complicate things for simpler and get it started scenarios. The other is the fact that potentially very large data set will be stored, where simply using a shared storage is an overhead that is unacceptable.
...
p. The cluster state, which includes the indices created, mapping applies and other meta information is versioned and stored on the nodes. In order to recovery it, the @gateway.recovery_after_nodes@ (or @gateway.recovery_after_master_nodes@) should be set to high enough value out of the total expected cluster size.
