...
p. The memcached module is allows to expose ElasticSearch "REST API":/docs/elasticsearch/rest_api over the memcached protocol (as closely as possible). 
...
p. The memcached module is an abstraction on top of an actual implementation of memcached. The following are supported:
