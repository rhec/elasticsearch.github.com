...
p. The @memory@ type stores the index in memory with high level of configuration options:
...
h2. ByteBuffer (memory)

p. The @bytebuffer@ type stores the index in memory using byte buffers, mainly allowing to use the *direct* option of byte buffer thus storing the index *outside of the JVM heap space*. The following settings are provided:

|_. Setting |_. Description |
|index.store.bytebuffer.direct|Should byte buffers be direct (allocated outside of the JVM heap space) or not. Defaults to @true@.|
|index.store.bytebuffer.bufferSize|A size based setting allowing to configure the buffers (chunks) that each file is fragmented to. Defaults to @1k@.|
|index.store.bytebuffer.cacheSize|A size based setting of the size of cached, already allocated buffers. Defaults to @20m@.|
|index.store.bytebuffer.warmCache|Should the cache be warmed or not. Defaults to @true@.|


h2. RAM

p. The @ram@ type stores the index in memory (maps to Lucene @RAMDirectory@). Note, both the "Memory":#Memory and "ByteBuffer":#ByteBuffer will probably provide better alternatives for in memory storage.
