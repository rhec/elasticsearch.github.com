...
p. Analyzers in general are broken down into a "Tokenizer":tokenizer with zero or more "Token Filter":tokenfilter applied to it. The analysis module allows to register @TokenFilters@, @Tokenizers@ and @Analyzers@ under logical names which can then be referenced either in mapping definitions or in certain APIs. The Analysis module automatically registers (*if not explicitly defined*) built in analyzers, token filters, and tokenizers. 
