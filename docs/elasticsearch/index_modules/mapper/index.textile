...
p. The mapper module acts as a registry for the type mapping definitions added to an index using the "create mapping":/docs/elasticsearch/json_api/admin/indices/create_mapping. It also handles the dynamic mapping support for types that have no explicit mappings pre defined. For more information about mapping definitions, check out the "mapping section":/docs/elasticsearch/mapping.
