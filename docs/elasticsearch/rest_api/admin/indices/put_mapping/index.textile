...
p. When an existing mapping already exists under the given type, the two mapping definitions, the one already defined, and the new ones are merged. The @ignoreConflicts@ parameters can be used to control if conflicts should be ignored or not, by default, it is set to @true@ which means conflicts are ignored.
