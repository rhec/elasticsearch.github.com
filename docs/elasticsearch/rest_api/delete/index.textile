---
layout: doc_es
title: ElasticSearch Docs | JSON Delete API
---
...
p. The delete operation automatically creates an index if it has not been created before (check out the "create index API":../admin/indices/create_index for manually creating an index), and also automatically creates a dynamic type mapping for the specific type if it has not been created before (check out the "create mapping API":../admin/indices/create_mapping for manually creating type mapping). 
